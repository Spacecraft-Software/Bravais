...
|   |   +-- cosmic-unmax.nix       # steelbore-cosmic-unmax: the same, for COSMIC
...
**Extensions (17):**
  caffeine, just-perfection, window-gestures, wayland-or-x11, toggler, vim-alt-tab, open-bar, tweaks-in-system-menu, launcher, window-title-is-back, yakuake, forge, tiling-shell, smart-tiling, simple-tiling, warp-toggle, resource-monitor
...
**Self-maximizing clients under COSMIC** (`modules/desktops/cosmic-unmax.nix`): the same user-visible problem, and COSMIC offers no setting for it — `CosmicCompConfig` (cosmic-comp 1.0.13) carries autotile, active_hint, focus_follows_cursor, edge_snap_threshold, input, xkb, workspaces, zoom and appearance, and nothing about maximization. Nor is it COSMIC's choice: the client asks via xdg-toplevel `set_maximized`, cosmic-comp records `pending.maximized = true` (`src/wayland/handlers/xdg_shell/mod.rs`) and honours it at map time (`src/shell/mod.rs`), with no window rule able to refuse. Ships **steelbore-cosmic-unmax** (`pkgs/steelbore-cosmic-unmax/`; Rust + wayland-client, GPL-3.0), a systemd **user** service bound to `cosmic-session.target`. A sibling of the niri daemon rather than one binary with a backend switch, because the transports share nothing: niri exposes a JSON IPC socket, COSMIC exposes Wayland protocols. Detection is consequently simpler and more reliable here — `zcosmic_toplevel_handle_v1.state` reports `maximized` explicitly, so no geometric inference and no `gaps > 0` precondition. Fullscreen is exempt for the same reason as on niri. The three protocol XMLs are **vendored** under `pkgs/steelbore-cosmic-unmax/protocols/` (from pop-os/cosmic-protocols rev `160b086`, the rev cosmic-comp 1.0.13 pins) and compiled with `wayland-scanner`, so the crate resolves from crates.io alone and needs no `cargoLock.outputHashes`. Pre-existing windows are settled after one initial Wayland roundtrip: without that, every window already open looks like it just appeared (the handle — and so `first_seen` — is created at daemon startup) and a window maximized hours earlier would be reverted seconds after login.
...
| COSMIC Term    | `/etc/cosmic/com.system76.CosmicTerm/v1/profiles` (Nushell shell; system default) | RON |
...
**WarCraft II:** wargus (C++ — Stratagus engine reimplementation), **overridden**. `pkgs.wargus` does not build: its `postInstall` runs `wartool` over a full commercial rip of the game that the derivation fetches from archive.org (marked `license = unfree` there), and that URL has 403'd since ~2026. Re-pointing it at another mirror would keep the tier-3 violation, so the override drops the fetch and builds the engine alone. Nothing is lost — the launcher resolves data at **run** time to `$HOME/.stratagus/data.Wargus` (`SetUserDataPath` in `stratagus-game-launcher.h`), a user-writable path, so the store copy the derivation built was never the one an install reads. Import via `play-warcraft2 --extract`, which accepts the DOS `REZDAT.WAR`, the Battle.net Edition `INSTALL.MPQ`/`INSTALL.EXE`, or a GOG installer `.exe` (`GAME_CD` in `wargus.cpp`).

**Wine (`steelbore.packages.games.wine.enable`, default off):** wineWow64Packages.staging (C), winetricks (shell), lutris (Python) — for the Blizzard titles with no source port (StarCraft, StarCraft Remastered, WarCraft III / Reforged). A separate toggle for the same reason Steam has one: measured **665 MiB down / 2.8 GiB unpacked**. `wineWow64Packages`, *not* `wineWowPackages` — nixpkgs deprecates the latter and evaluating it prints that warning on every eval of the system, the same leak the `nil` formatter override exists to stop. This toggle supplies the runtime only; the games install into a prefix through Lutris and self-update there, out-of-band like the other vendor-updated software.

**Commands:** every launcher is `play-`-prefixed — `play-doom`, `play-quake`, `play-quake-vk`, `play-duke3d`, `play-duke3d-eduke32`, `play-wolf3d`, `play-warcraft2`, and one `play-<slug>` per DOS game. The prefix is load-bearing, not cosmetic: `freedoom`'s own `bin/freedoom1` launcher searches `PATH` for a port literally named `doom`, so a wrapper by that name would hijack it — and because `config.system.path` is a `buildEnv` with `ignoreCollisions = true`, a name clash with an engine binary would silently pick a winner rather than failing the build the way the Home Manager `buildEnv` does (constraint #12).
