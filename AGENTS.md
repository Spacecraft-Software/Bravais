...
39. **Every USB device re-enumerates on S3 resume, and fprintd never re-scans -- so a fingerprint reader that worked before a suspend is dead after one** -- fprintd enumerates devices once at daemon startup. Resume from S3 gives the reader a new USB device number, so a daemon that was already running holds a handle to a device that no longer exists and fails every identify instantly. At the COSMIC lock screen that shows as the fingerprint prompt appearing and being withdrawn about a second later, falling back to the password field. The correlation is exact and easy to confirm: every `cosmic-greeter: pam_fprintd(...): ReleaseDevice failed: ... has been removed from the system` in the journal shares its **second** with a `kernel: ACPI: PM: Waking up from system sleep state S3`. It also explains the "but it worked once" report -- fprintd idle-exits ~30 s after its last client, so an attempt made more than half a minute after resuming gets a fresh daemon, enumerates the current device and succeeds. **This is a SECOND, independent cause from the USB-autosuspend one** the `06cb:00bd` udev rule in `modules/hardware/fingerprint.nix` addresses; that rule works (`power/control` reads `on`) and is unrelated, because S3 re-enumerates regardless of runtime power policy. Do not re-diagnose the autosuspend rule when this recurs. The fix is `powerManagement.powerDownCommands` + `resumeCommands` in that module, both merely **stopping** fprintd: it is D-Bus activated, so the next PAM attempt starts a fresh one once the bus has settled, whereas starting one in the hook would race the re-enumeration and cache the very failure being avoided. Stopping on the way *into* sleep as well as out is what closes the window where the greeter re-arms faster than the resume hook runs.

40. **`strip` silently corrupts a large `autoPatchelfHook`'d binary, and the blast radius is 16 bytes** -- `pkgs/github-copilot-app` ships a 369 MB PIE with 155 677 dynamic relocations. autoPatchelfHook must relocate `.dynstr`, `.rela.dyn` and `.dynamic` into fresh LOAD segments at the end of the file to fit the store RUNPATH, and the `strip -S` nixpkgs' `fixupPhase` runs writes symbol-string bytes across the **start** of the moved `.rela.dyn` -- measured, exactly the first 16 bytes, i.e. `r_offset` and `r_info` of entry 0, leaving that entry's addend and all 155 676 following entries intact. glibc walks the first `DT_RELACOUNT` entries down a fast path asserting every one is `R_X86_64_RELATIVE`, so a single bad entry at index 0 aborts before `main`: `Inconsistency detected by ld.so: ../sysdeps/x86_64/dl-machine.h: 498: elf_machine_rela_relative: Assertion ... failed!`. Launched from the app menu it just does nothing. **Nothing fails at build time and every static check looks clean** -- `readelf -d` is well-formed, RELA lands exactly inside the last LOAD segment, and the damage is 16 bytes in a 369 MB file. `dontStrip = true;` is the fix, at a measured cost of ~634 MB (369 MB stripped -> 1003 MB). Confirm the diagnosis rather than guessing at it: read the RELA table at `DT_RELA` and find the first entry whose type is not 8, then reconstruct entry 0 from its neighbours (the entries step `r_offset += 8`, `r_addend += 0x10`, so the correct values are unambiguous) and patch those 16 bytes in a copy -- if the app then starts, it is this. To prove `strip` is the writer rather than patchelf, run `strip -S` on the repaired copy and re-read the entry; the garbage that lands there is plainly ASCII from the symbol table. **Suspect this for any vendored binary big enough that patchelf has to move sections** -- it is not specific to Copilot, and the failure gives no build-time signal at all.
