...
34. **Touchpad gesture finger counts are hardcoded in both compositors, and unifying them costs a source build** -- neither niri nor cosmic-comp exposes a finger count in config. niri's `gestures {}` block has only `dnd-edge-view-scroll`, `dnd-edge-workspace-switch` and `hot-corners`; `CosmicCompConfig` has no gesture field at all. The bindings live in source: niri `src/input/mod.rs` uses `event.fingers() == 3` for workspace switching and `== 4` for the Overview, while cosmic-comp `src/input/mod.rs` has `match gesture_state.fingers { 3 => None, // TODO: 3 finger gestures  4 => { ...NextWorkspace/PrevWorkspace... } }`. Bravais unifies on **3** via an overlay in `modules/core/nix.nix` that makes cosmic-comp's 3 share its 4 arm. Unifying on 4 was rejected deliberately: it would collide with niri's Overview gesture and would ALSO need a source patch, of the compositor whose 4-finger arm is already useful -- whereas cosmic-comp's 3-finger arm is an unused upstream TODO. Two things to know before touching it. The patch is **two single-line `--replace-fail` substitutions**, and the order matters: the `3 => None` arm must be deleted BEFORE `4 => {` becomes `3 | 4 => {`, or the new arm sits behind a live `3 =>` and never matches; deleting it is safe because the match already ends in a catch-all `_ => None`. And the overlay **forces cosmic-comp to build from source**, losing the binary cache for a large Rust package -- that is the entire price of the feature, and a cosmic-comp version bump can move the lines it targets, at which point `--replace-fail` fails the build loudly rather than silently reverting to 4-finger-only.
