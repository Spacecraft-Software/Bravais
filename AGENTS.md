...
modules/desktops/          # gnome, cosmic, plasma, niri, leftwm (+ shared.nix dunst, assertions.nix guards,
                           #   niri-unmax.nix, gnome-mouse-nav.nix side-button workspace nav)
...
29. **GNOME cannot bind a mouse button, so the side-button workspace nav is an evdev remap -- and it MUST stay session-scoped** -- Mutter's keybinding schemas (`org.gnome.desktop.wm.keybindings`) accept keyboard accelerators only; there is no `MouseBack` equivalent, and no nixpkgs GNOME extension binds side buttons (`gnomeExtensions.panel-scroll` is scroll-on-the-panel only). Under Wayland an extension cannot reliably grab pointer buttons over an application window either. `modules/desktops/gnome-mouse-nav.nix` therefore remaps `BTN_SIDE`/`BTN_EXTRA` **below the compositor** with `xremap` (Rust) into `<Control><Alt>Left/Right`, which GNOME already binds to `switch-to-workspace-left/-right` and grabs globally -- which is why the module needs no dconf settings and why no application can observe the emitted combo. **The unit is bound to `gnome-session-initialized.target`, not `graphical-session.target`, and that is load-bearing**: a system-wide or graphical-session remapper would swallow these buttons before niri ever saw them, breaking the native `MouseBack`/`MouseForward` binds in `users/mj/niri.nix` and killing browser back-forward in every session at once instead of only under GNOME. Two further non-obvious points: xremap `EVIOCGRAB`s every device it listens to and re-emits through a virtual one, so while the unit runs **all** keyboard and mouse input is proxied through it (the kernel releases grabs on process death, and `Restart=on-failure` covers a crash); and `--watch=device` is required on a laptop or a Bluetooth mouse that reconnects after suspend is never picked up. The `uinput` group membership in `users/mj/default.nix` is what lets it create that virtual device -- `input` covers only *reading* the real ones -- and a group change needs a re-login, not just a rebuild. Note niri's side buttons deliberately do **columns** (`focus-column-left/right`), not workspaces: niri's workspaces stack vertically and it has no horizontal workspace axis, so matching GNOME's direction means binding the horizontal thing niri does have.
