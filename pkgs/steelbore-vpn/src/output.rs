...
// third of the answer.
//
// DUPLICATION, KNOWINGLY: this is a near-copy of pkgs/preflight/src/output.rs.
// The right home is a shared `steelbore-cli` crate, but each package under
// pkgs/ builds from `lib.cleanSource ./.`, so a path dependency outside the
// crate directory does not survive the Nix build -- lifting it needs a Cargo
// workspace spanning pkgs/, which is a refactor of its own and would drag
// preflight's lockfile with it. Tracked in TODO.md. Until then: a change to the
// envelope, the severity ladder or the detection cascade belongs in BOTH files.
...
#[expect(
    dead_code,
    reason = "Usage is part of the exit-code contract that `steelbore-vpn schema` publishes, but clap emits 2 itself so no code path here constructs it"
)]
pub enum Exit {
    Success = 0,
    Failure = 1,
